...
#  db.Column('VenueId',db.Integer,db.ForeignKey('Venue.id'),primary_key = True),
...
def show_venue(venue_id):
  ...
  for artist in GetVenuesById.Artists:
      datetime_object = datetime.strptime(artist.startTime, '%Y-%m-%d %H:%M:%S')
...
def show_artist(artist_id):
  ...
  UpcomingShowsCount = 0
  CurrentTime = datetime.now()
  for venue in GetArtistById.Venues:
      datetime_object = datetime.strptime(venue.startTime, '%Y-%m-%d %H:%M:%S')
      if datetime_object > CurrentTime:
        UpcomingShowsCount +=1
        upcoming_shows.append(
          {
